...
use crate::domain::usuario::password_hasher::PasswordHasher;
use crate::domain::usuario::role::Role;
use crate::domain::usuario::user_id::UserId;
...
/// esta satisfeita.
///
/// `Clone` foi adicionado nesta semana porque o cache em memoria
/// (`infra::cache_user_repository`) guarda o `User` inteiro dentro de um
/// `HashMap` protegido por `RwLock`: cada leitura devolve uma copia, nunca
/// uma referencia viva ao dado guardado no cache (evita segurar o lock
/// mais tempo que o necessario).
#[derive(Clone)]
pub struct User {
    id: UserId,
    email: Email,
...
impl User {
    ...
    /// Fluxo interno/administrativo (seed, seed de admin, tela de gestao
    /// de usuarios via `PUT /admin/usuarios/:id`). Fica explicito no nome
    /// que quem chama esta decidindo conscientemente o papel do usuario.
    ...
    /// Regra de negocio de atualizacao cadastral (`UpdateUser`, novo na
    /// Semana 10): troca o email mantendo identidade, senha e papel
    /// intactos. Devolve um novo `User` (o agregado continua imutavel de
    /// fora) - quem chama e quem decide persistir o resultado.
    pub fn with_email(&self, new_email: Email) -> Self {
        Self {
            id: self.id,
            email: new_email,
