...
use crate::application::dto::{AuthenticateUserInput, AuthenticateUserOutput};
...
/// Caso de uso de login. Observação de segurança (regra de negócio
/// implícita): tanto "email não existe" quanto "senha errada" resultam no
/// mesmo `DomainError::InvalidCredentials`, para não vazar quais emails
/// estão cadastrados.
...
impl AuthenticateUser {
    ...
    fn issue_token_for(
        &self,
        user: &User,
    ) -> Result<crate::application::token_service::AccessToken, ApplicationError> {
        self.tokens
            .issue(user.id())
