...
use crate::application::audit_logger::{AuditEntry, AuditLogger};

/// Adapter concreto do port `AuditLogger`: grava cada `AuditEntry` como
/// uma linha num arquivo texto (`auditoria.txt` por padrao), funcionando
/// como o log de auditoria pedido na Semana 10 - toda vez que um insert,
/// update ou delete acontece em `User`/`Cliente`, uma linha nova aparece
/// aqui, com o tipo da operacao.
///
