...
use cadastro_api::application::shared::command_bus::CommandBus;
use cadastro_api::application::shared::query_bus::QueryBus;
...
/// que toda escrita vira uma linha no arquivo de auditoria, que esse
/// mesmo arquivo e lido de volta no boot para reidratar o cache (Semana
/// 11), e agora (Semana 13) que os 12 casos de uso de escrita e os 7 de
/// leitura sao agrupados em um `CommandBus`/`QueryBus` antes de chegar na
/// presentation. Nada fora daqui conhece `Cache*Repository`,
/// `Audited*Repository`, `FileAuditLogger`, `AuditLogReplayer` ou os
...
/// Monta o `CommandBus` e o `QueryBus` (Semana 13) e devolve o `AppState`
/// que a presentation efetivamente consome. Os 19 casos de uso continuam
/// existindo e sendo instanciados aqui - o unico lugar que mudou de
/// "conhecer 19 `Arc<UseCase>`" para "conhecer 2 barramentos" foi o
/// `AppState`/a presentation; o composition root ainda precisa conectar
/// cada caso de uso ao seu repositorio, exatamente como antes.
